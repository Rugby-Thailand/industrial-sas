...
const textNumber = (value: number | undefined, divisor = 1) =>
  value === undefined ? "" : String(value / divisor);
function initialDraft(_product: Product, batch?: Batch): Draft {
  const rows = batch
    ? batch.packages.map((unit) => ({
        ...newRow(textNumber(unit.quantity)),
...
const packageDraft = (r: Row): PackageDraft => ({
  ...(r.quantity.trim() ? { quantity: Number(r.quantity) } : {}),
  ...(r.length.trim() ? { lengthMm: mm(r.length) } : {}),
  ...(r.width.trim() ? { widthMm: mm(r.width) } : {}),
  ...(r.height.trim() ? { heightMm: mm(r.height) } : {}),
  ...(r.weight.trim() ? { weightKg: Number(r.weight) } : {}),
...
function PackingForm({
  ...
  const issue = validatePacking(
    ...
    packages.map((p) => ({
      ...
      dimensionsChecked: p.dimensionsChecked,
    })),
    product.unit,
  );
  const preview =
                  ...
                  <div className="grid gap-4 sm:grid-cols-3">
                    {(["length", "width", "height"] as const).map((field) => (
                      <Field
                        ...
                        type="number"
                        min={draft.displayUnit === "m" ? 0.001 : 0.1}
                        max={draft.displayUnit === "m" ? 100 : 10000}
                        step={draft.displayUnit === "m" ? "0.001" : "0.1"}
                        required
                      />
                    ))}
                  </div>
