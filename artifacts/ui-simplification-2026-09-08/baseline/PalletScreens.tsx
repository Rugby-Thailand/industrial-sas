...
function correctionPath(detail: PalletDetail) {
...
export function Summary({ detail }: { detail: PalletDetail }) {
  const { tr } = useUnitText(
    detail.pallet.storageFormat ?? detail.product?.storageFormat,
  );
  return (
    <div className="mb-4 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-xl border border-border bg-surface px-4 py-3 text-sm">
      <span className="font-mono font-semibold">{detail.pallet.code}</span>
      <span>
        {detail.product?.sku} · {detail.product?.name}
      </span>
      <span>
        {detail.pallet.quantity} {detail.product?.unit}
      </span>
      ...
      detail.pallet.heightMm ? (
        ...
        >
          {mmText(detail.pallet.lengthMm)} × {mmText(detail.pallet.widthMm)} ×{" "}
          {mmText(detail.pallet.heightMm)}
        </span>
      ) : null}
      {detail.pallet.lot ? (
        <span>
          {tr("Lot", "ล็อต")} {detail.pallet.lot}
        </span>
      ) : null}
      <Status
...
function Measurement({
  ...
  async function save(find: boolean) {
    await op.run(async () => {
      ...
      router.push(find ? storagePath(pallet._id) : palletPath(pallet._id));
    });
  }
  ...
  return (
    <>
      <Heading
        ...
        back={`${productPath(pallet.productId)}?resumePalletId=${encodeURIComponent(pallet._id)}`}
...
function Recommendations({
  ...
  const candidates = [...validCandidates, ...(showUnavailable ? previews : [])];
  ...
  const current =
    candidates.find((c) => keyOf(c) === selected) ?? candidates[0];
  return (
    <>
      <Heading
        title={tr("Recommended storage", "พื้นที่จัดเก็บที่แนะนำ")}
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3 text-sm">
        {(detail.pallet.storageFormat ?? detail.product?.storageFormat) ===
          ...
          <Button
            ...
            onClick={() => {
              setIncludeStacking(!includeStacking);
              setSelected("");
            }}
          >
        ...
        <div className="grid items-start gap-4 2xl:grid-cols-[230px_1fr]">
          <section className="min-w-0 space-y-3">
            ...
            <div className="relative">
              ...
              />
              <Input
                aria-label={tr("Search storage locations", "ค้นหาจุดจัดเก็บ")}
                placeholder={tr(
            ...
            {filteredCandidates.length === 0 ? (
              ...
              <div
                role="region"
                aria-label={tr("Location options", "ตัวเลือกจุดจัดเก็บ")}
                tabIndex={0}
              ...
              >
                {filteredCandidates.map((c) => (
                  <button
                    key={keyOf(c)}
                    type="button"
                    aria-pressed={keyOf(current) === keyOf(c)}
                    onClick={() => setSelected(keyOf(c))}
                    className={`min-w-52 shrink-0 rounded-xl border p-3 text-left transition outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-inset 2xl:min-w-0 ${keyOf(current) === keyOf(c) ? "border-accent bg-accent/10" : "border-border bg-surface hover:border-accent/60"}`}
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      <MapPin
                        ...
                        aria-hidden="true"
                      />
                      <span className="font-semibold">{c.locationName}</span>
                      {validCandidates[0] &&
                      keyOf(c) === keyOf(validCandidates[0]) ? (
                    ...
                    <p className="mt-2 text-xs text-muted">
                      {c.buildingCode} · {tr("Floor", "ชั้น")} {c.floorNumber}
                    </p>
                    <p className="mt-2 text-xs text-muted">
                      {mmText(c.support.widthMm)} × {mmText(c.support.depthMm)}
                    </p>
                    <p className="mt-2 text-xs text-muted">
                      {c.supportLabel ??
                        c.supportCode ??
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm font-medium break-words">
        {destination.buildingCode} <span className="text-muted">→</span>{" "}
        {tr("Floor", "ชั้น")} {destination.floorNumber}{" "}
        <span className="text-muted">→</span> {destination.locationName}{" "}
      ...
      <p className="text-xs text-muted">
        {tr(
          "X/Y are measured from the marked origin inside this location.",
          "พิกัด X/Y อ้างอิงจากจุดเริ่มต้นที่แสดงภายในจุดจัดเก็บนี้",
        )}
      </p>
    </div>
  );
...
export function PlacementEditor({
  ...
  const [requested, setPlacement] = useState<PalletPlacement>({
  ...
  function updateCoordinate(axis: "x" | "y", value: string) {
    ...
    if (value.trim() !== "" && Number.isFinite(Number(value)))
      setPlacement((previous) => ({
        ...previous,
        [axis === "x" ? "xMm" : "yMm"]: Math.round(Number(value) * 1000),
      }));
  }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <aside className={`${panel} space-y-4`}>
            ...
            <>
              <div className="space-y-2 border-t border-border pt-4 text-sm">
                {[
                  ...
                  <p
                    key={label}
                    className={`flex items-center gap-2 ${issue ? "text-warning" : "text-success"}`}
                  >
                    {issue ? (
          ...
          >
            {moving
              ? tr("Review move", "ตรวจสอบการย้าย")
              : tr("Confirm spot", "ยืนยันจุดจัดเก็บ")}
          </Button>
          {unchanged ? (
            <Notice
              title={tr(
        ...
        <DialogContent closeLabel={tr("Close", "ปิด")}>
          <DialogHeader>
            <DialogTitle>
              {moving
                ? tr("Review move", "ตรวจสอบการย้าย")
                : tr("Confirm storage destination", "ยืนยันปลายทางจัดเก็บ")}
            </DialogTitle>
            <DialogDescription>
              {detail.pallet.code} · {detail.product?.sku} ·{" "}
              {detail.pallet.quantity} {detail.product?.unit}
            </DialogDescription>
          </DialogHeader>
          {moving ? (
            <p className="text-sm text-muted">
              {tr("Pallet dimensions", "ขนาดพาเลท")}:{" "}
              {mmText(detail.pallet.lengthMm ?? 0)} ×{" "}
              {mmText(detail.pallet.widthMm ?? 0)} ×{" "}
              {mmText(detail.pallet.heightMm ?? 0)}
            </p>
          ) : null}
          {moving && detail.destination && detail.placement ? (
            <section className="space-y-2">
              <h3 className="font-semibold">{tr("From", "จาก")}</h3>
              <DestinationSummary
                destination={detail.destination}
                placement={detail.placement}
              />
              <h3 className="font-semibold">{tr("To", "ไปยัง")}</h3>
            </section>
          ) : null}
          <DestinationSummary destination={candidate} placement={placement} />
          {moving ? (
          ...
          <DialogFooter>
            ...
            <Button
              disabled={op.busy || !!issue}
              onClick={() => void reserve()}
            >
              {op.busy
                ? tr("Reserving…", "กำลังจอง…")
                : tr("Reserve spot", "จองจุดนี้")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function PalletDetailScreen({
  ...
  return (
    <>
      ...
      {!scanner && !cancel ? <ErrorNotice message={op.error} /> : null}
      ...
      <div className="grid items-start gap-5 xl:grid-cols-[1.3fr_1fr]">
        ...
        <div className="space-y-4">
          ...
          {reserved && canManage ? (
            <section className={`${panel} space-y-4`}>
              <h2 className="font-semibold">
                {tr("Placement instructions", "ขั้นตอนการจัดเก็บ")}
              </h2>
              <ol className="list-decimal space-y-2 pl-5 text-sm text-muted">
                <li>
                  {tr(
                    "Move the pallet to the named location.",
                    "นำพาเลทไปยังจุดจัดเก็บที่ระบุ",
                  )}
                </li>
                <li>
                  {tr(
                    "Scan the destination label or verify its code manually.",
                    "สแกนป้ายปลายทางหรือตรวจสอบรหัสด้วยตนเอง",
                  )}
                </li>
                <li>
                  {tr(
                    "Place it at the highlighted X/Y position.",
                    "วางพาเลทตามพิกัด X/Y ที่แสดง",
                  )}
                </li>
                <li>
                  {tr(
                    "Confirm stored after physical placement.",
                    "ยืนยันจัดเก็บหลังวางพาเลทจริง",
                  )}
                </li>
              </ol>
              {verified ? (
                <p className="flex items-center gap-2 text-sm text-success">
                  ...
                  {placement?.verificationMethod === "MANUAL"
                    ? tr(
                        "Destination verified manually",
                        "ตรวจสอบปลายทางด้วยตนเองแล้ว",
                      )
                    : tr("Destination QR verified", "ตรวจสอบ QR ปลายทางแล้ว")}
                </p>
              ) : null}
              <Button className="w-full" onClick={() => setScanner(true)}>
                <ScanLine className="size-4" aria-hidden="true" />
                {verified
                  ? tr("Confirm physical placement", "ยืนยันการวางจริง")
                  : placement?.supportPalletId
                    ? tr("Verify pallet identities", "ตรวจสอบรหัสพาเลททั้งสอง")
                    : tr("Scan destination QR", "สแกน QR ปลายทาง")}
              </Button>
            </section>
          ) : null}
        </div>
      ...
      <Dialog open={scanner && canManage && reserved} onOpenChange={setScanner}>
        <DialogContent closeLabel={tr("Close", "ปิด")}>
          <DialogHeader>
            <DialogTitle>
              {tr(
            ...
            <DialogDescription>
              {destination?.locationName} · {placement?.positionCode}
            </DialogDescription>
          </DialogHeader>
          <DestinationScanner
            storageFormat={
          ...
          {placement?.supportPalletId ? (
            <div className="space-y-3">
              ...
              <Field
                label={tr("Upper pallet code or QR", "รหัสหรือ QR พาเลทด้านบน")}
                value={upperCode}
          ...
          {destination && coords ? (
            <DestinationSummary destination={destination} placement={coords} />
          ) : null}
          <p className="text-sm text-muted">
            {tr(
          ...
          <ErrorNotice message={op.error} />
          <DialogFooter>
            <Button variant="outline" onClick={() => setScanner(false)}>
              {tr("Close", "ปิด")}
            </Button>
            <Button
              disabled={
