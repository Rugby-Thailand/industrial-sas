...
describe("exact storage recommendations", () => {
  ...
  it("edits immediately and rejects empty coordinates without silently using zero", () => {
    ...
    expect(screen.getByRole("button", { name: "Confirm spot" })).toBeDisabled();
    expect(
      screen.queryByText("Dimensions fit inside the support"),
    ).not.toBeInTheDocument();
    number("X (m)", "0.3");
    expect(screen.getByRole("button", { name: "Confirm spot" })).toBeEnabled();
  });
  ...
  it.each([
    ...
    (_name, size, blockedReason, message) => {
      ...
      expect(
        screen.getByRole("button", { name: "Confirm spot" }),
      ).toBeDisabled();
      expect(callsFor("reserve")).toHaveLength(0);
    },
  );
  ...
  it("previews and reviews a local coordinate without reserving until explicit Reserve", async () => {
    renderPallet("storage");
    expect(mocks.write).not.toHaveBeenCalled();
    expect(
    ...
    const dialog = screen.getByRole("dialog", {
      name: "Confirm storage destination",
    });
    expect(dialog).toHaveTextContent("BLDG-A");
    expect(dialog).toHaveTextContent("FG-1");
    expect(mocks.write).not.toHaveBeenCalled();
    fireEvent.click(
      within(dialog).getByRole("button", { name: "Reserve spot" }),
    );
    await waitFor(() =>
      expect(mocks.push).toHaveBeenCalledWith(
        "/finished-goods/pallets/pallet-a",
  ...
  it("keeps the confirmation dialog and proposed position on reservation conflict", async () => {
...
describe("reserved destination and physical storage confirmation", () => {
  ...
  it("keeps storage confirmation disabled after a wrong destination", async () => {
    ...
    renderPallet();
    fireEvent.click(
      screen.getByRole("button", { name: "Scan destination QR" }),
    );
    const dialog = screen.getByRole("dialog", {
...
describe("read-only pallet access", () => {
  ...
  it.each(["measure", "storage"] as const)(
    ...
    (view) => {
      ...
      noEnabledButton("Confirm spot");
      noEnabledButton("Reserve spot");
      expect(mocks.write).not.toHaveBeenCalled();
    },
  );
...
it("guides stacked reservations to verify both pallet identities", () => {
  ...
  renderPallet("detail");
  fireEvent.click(
    screen.getByRole("button", { name: "Verify pallet identities" }),
  );
  expect(
