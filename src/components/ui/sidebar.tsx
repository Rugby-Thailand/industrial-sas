    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          side={side}
          {...(mobileCloseLabel === undefined
            ? {}
            : { closeLabel: mobileCloseLabel })}
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{mobileTitle}</SheetTitle>
            {mobileDescription === undefined ? null : (
...
/**
 * The only affordance that opens and closes the navigation.
 *
 * `label` is required and untranslated copy cannot be forgotten, because there
 * is no English default to fall back to. `size="icon"` rather than the
 * registry's `icon-sm`: this control appears in the handheld header too, so it
 * clears 48 by 48 like every other target.
 */
