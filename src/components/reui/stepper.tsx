...
function Stepper({
  ...
  const registerTrigger = useCallback((node: HTMLButtonElement) => {
    ...
    return () => {
      setTriggerNodes((prev) => prev.filter((entry) => entry !== node));
    };
  }, []);
...
function StepperTrigger({
  ...
  // Register this trigger for keyboard navigation without reading a mutable
  // ref during render. The registry cleanup also prevents stale hidden steps.
  ...
  const myIdx = triggerNode === null ? -1 : triggerNodes.indexOf(triggerNode);
