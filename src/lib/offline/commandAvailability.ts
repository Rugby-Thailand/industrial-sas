 ...
 * preview mode and an unconfigured deployment, and the code-owned dispatch
 * decision (`convex/model/platform/commandClassification.ts`), which knows
 * whether a command may be deferred.
 *
 * The mapping that matters is `PREVIEW`. Preview data is not a server, so a
 * screen rendering synthetic rows must not report a posting as ready to send —
 * `isServerHealthy` already refuses to call preview healthy, and this keeps
 * that judgement in one place rather than letting each screen decide.
 */
...
export function connectionStateOf(status: ConnectionStatus): ConnectionState {
  switch (status) {
    ...
    case "PREVIEW":
      /*
       * Both are "we do not know that the server is there". `CONNECTING` is
       * literally that; `PREVIEW` is a screen showing synthetic rows, where a
       * blocked command must stay blocked and a queueable one is held rather
       * than pretend-sent.
       */
      return "UNKNOWN";
  }
