...
import {
  batchTestResponse,
  catalogueTestResponse,
} from "./paginationTestAdapter";
...
vi.mock("@/hooks/useScanContinuation", async (importOriginal) => ({
  ...(await importOriginal<typeof ScanContinuationModule>()),
  ...
  useQuery: (ref: Parameters<typeof getFunctionName>[0], args: unknown) =>
    catalogueTestResponse(
      ...
      batchTestResponse(
        getFunctionName(ref),
        mocks.query(
          /:(pageProductBatches|pageLegacyUnits|productSummary)$/.test(
            getFunctionName(ref),
          )
...
describe("preparation batches and legacy review", () => {
  ...
  it("protects reserved and moving records and keeps read-only batch actions hidden", () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Edit available units atch-one" }),
    ).not.toBeInTheDocument();
    expect(
  ...
  it("shows Thai batch counts and loads packing history on demand", async () => {
    renderBatches(
      {
        batches: [
          {
    ...
    expect(screen.getByText(/100 pieces · 1 กล่อง/)).toBeVisible();
    expect(screen.queryByText(/OTHER/)).not.toBeInTheDocument();
    expect(
      screen.queryByText("ประวัติการแบ่งบรรจุ (1)"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "ดูการจัดเก็บ atch-one" }),
    ).toBeVisible();
  });
});
...
describe("direct catalogue unit actions", () => {
  it.each([
    ...
    ["RESERVED", "Continue placement", "/finished-goods/pallets/pallet-a"],
    ["STORED", "View details", "/finished-goods/pallets/pallet-a"],
  ] as const)(
    "shows the same explicit action for %s in cards and tables",
    (status, label, href) => {
      mocks.query.mockReturnValue(
        querySuccess({
          ...finishedGoodsList,
          pallets: [{ ...finishedGoodsList.pallets[0]!, status }],
        }),
      );
      renderWithIntl(<FinishedGoodsCatalogue />, {
        locale: "en",
      ...
      fireEvent.click(screen.getByRole("button", { name: "Storage units" }));
      expect(
        screen.getByRole("link", { name: `${label} P-001` }),
  ...
  it("skips the intermediate measurement screen for a preparation batch", () => {
    mocks.query.mockReturnValue(
      querySuccess({
        ...finishedGoodsList,
        pallets: [
          { ...finishedGoodsList.pallets[0]!, preparationBatchId: "batch-a" },
        ],
      }),
    );
    renderWithIntl(<FinishedGoodsCatalogue />, {
  ...
  it("resumes moves directly but keeps read-only users on unit details", () => {
    ...
    const view = renderWithIntl(<FinishedGoodsCatalogue />, {
      locale: "en",
      workspace: false,
    });
    fireEvent.click(screen.getByRole("button", { name: "Storage units" }));
    expect(
      screen.getByRole("link", { name: "Continue move P-001" }),
    ...
    expect(
      screen.queryByRole("link", { name: "Continue move P-001" }),
    ).not.toBeInTheDocument();
    expect(
...
describe("server catalogue pages", () => {
  const records = Array.from({ length: 41 }, (_, index) => ({
    ...finishedGoodsList.products[0]!,
    _id: `paged-${index}`,
    sku: `PAGE-${String(index).padStart(3, "0")}`,
    name: `Paged product ${index}`,
    updatedAt: 1000 - index,
  }));
  it("shares page position between cards and tables and restores it after opening details", () => {
    mocks.query.mockReturnValue(
      querySuccess({ products: records, pallets: [] }),
    );
    const first = renderWithIntl(<FinishedGoodsCatalogue />, {
      locale: "en",
      workspace: false,
    });
    expect(screen.getByText("Paged product 0")).toBeVisible();
    expect(screen.queryByText("Paged product 20")).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    expect(screen.getByText("Page 2")).toBeVisible();
    expect(screen.getByText("Paged product 20")).toBeVisible();
    fireEvent.click(screen.getByRole("button", { name: "Table view" }));
    expect(screen.getByText("Paged product 20")).toBeVisible();
    first.unmount();
    renderWithIntl(<FinishedGoodsCatalogue />, {
  ...
  it("searches all records, resets page size, and steps back after the last page is deleted", () => {
    mocks.query.mockReturnValue(
      querySuccess({ products: records, pallets: [] }),
    );
    const view = renderWithIntl(<FinishedGoodsCatalogue />, {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    fireEvent.click(screen.getByRole("button", { name: "Next page" }));
    expect(screen.getByText("Paged product 40")).toBeVisible();
    mocks.query.mockReturnValue(
    ...
    fireEvent.change(
      screen.getByRole("textbox", { name: "Search finished goods" }),
      { target: { value: "Paged product 39" } },
    );
    expect(screen.getByText("Page 1")).toBeVisible();
    expect(screen.getByText("Paged product 39")).toBeVisible();
    chooseOption("Records per page", "50");
    expect(screen.getByText("Page 1")).toBeVisible();
  });
});

it("continues a bounded scan without displaying an empty result", () => {
  const scanning = {
    status: "scanning",
    products: [],
    pallets: [],
  ...
  mocks.query.mockImplementation((name) =>
    querySuccess(name.endsWith(":page") ? scanning : finishedGoodsList),
  );
  const view = renderWithIntl(<FinishedGoodsCatalogue />, {
    locale: "en",
    workspace: false,
  ...
  expect(
    screen.queryByRole("heading", { name: "No matching records" }),
  ).not.toBeInTheDocument();
  expect(screen.getByRole("button", { name: "Next page" })).toBeDisabled();
  expect(screen.queryByText("Showing 0 records")).not.toBeInTheDocument();
  expect(mocks.query).toHaveBeenCalledWith(
    expect.stringContaining("catalogue:page"),
    expect.objectContaining({ scanCursor: "scan-next" }),
  );
...
it("reports an invalid first-page cursor without entering a reset loop", () => {
  ...
  mocks.query.mockImplementation((name) =>
    querySuccess(name.endsWith(":page") ? reset : finishedGoodsList),
  );
  renderWithIntl(<FinishedGoodsCatalogue />, {
    locale: "en",
    workspace: false,
  ...
  expect(
    screen.getByText("Records could not be loaded. Please try again."),
  ).toBeVisible();
  expect(
    screen.queryByRole("heading", { name: "No matching records" }),
  ).not.toBeInTheDocument();
  expect(mocks.query.mock.calls.length).toBeLessThan(12);
});
