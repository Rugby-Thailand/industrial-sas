...
export function PalletScene({
  ...
  const [view, setView] = usePreviewState(viewStateKey, {
    planView: false as boolean,
    quarterTurns: 0,
    zoom: 1,
  });
  const [annotations, setAnnotations] = useState(!area);
  const planView = view.planView;
  const quarterTurns = Math.abs(Math.trunc(view.quarterTurns)) % 4;
  const zoom = Math.min(2.5, Math.max(1, view.zoom));
  const setPlanView = (planView: boolean) =>
    setView((previous) => ({ ...previous, planView }));
  const setQuarterTurns = (next: number | ((value: number) => number)) =>
    setView((previous) => ({
      ...previous,
      quarterTurns: typeof next === "function" ? next(quarterTurns) : next,
    }));
  ...
  const paintBox = (
    ...
    if (colors.every((color) => color === "transparent")) {
      return (
        <SceneBox
          points={[...bottom, ...top]}
          mode={planView ? "plan" : "3d"}
          kind="package"
          held={dashed}
  ...
  return (
    <figure className="min-w-0 overflow-hidden rounded-xl border border-border bg-background">
      ...
      >
        ...
        {showMeasurements && annotations && area && (
          <g>
            <circle
              cx={origin.x}
              cy={origin.y}
