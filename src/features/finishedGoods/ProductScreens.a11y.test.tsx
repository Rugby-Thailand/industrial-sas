...
import { catalogueTestResponse } from "./paginationTestAdapter";
...
it.each(["en", "th"] as const)(
  "keeps the complete %s product form and summary accessible",
  async (locale) => {
...
it.each(["en", "th"] as const)(
  "keeps %s product and storage unit table views accessible",
  async (locale) => {
    const { container } = renderWithIntl(<FinishedGoodsCatalogue />, {
      locale,
      workspace: false,
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name: locale === "en" ? "Storage units" : "รายการหน่วยจัดเก็บ",
      }),
    );
    expect(await axe(container)).toHaveNoViolations();
  },
...
it.each(["en", "th"] as const)(
  "keeps %s header popovers and filter sheet accessible",
  async (locale) => {
    renderWithIntl(<FinishedGoodsCatalogue />, { locale, workspace: false });
    fireEvent.click(
      screen.getByRole("button", {
        name: locale === "en" ? "Table view" : "มุมมองตาราง",
      }),
    ...
    fireEvent.click(
      screen.getByRole("button", {
        name:
          locale === "en" ? "Filter and sort Product" : "กรองและเรียง สินค้า",
      }),
    );
    expect(await axe(screen.getByRole("dialog"))).toHaveNoViolations();
    fireEvent.click(
      screen.getByRole("button", {
        name: locale === "en" ? "Cancel" : "ยกเลิก",
      }),
    );
    fireEvent.click(
      screen.getByRole("button", {
        name: locale === "en" ? "Filters" : "ตัวกรอง",
      }),
    );
    expect(await axe(screen.getByRole("dialog"))).toHaveNoViolations();
  },
