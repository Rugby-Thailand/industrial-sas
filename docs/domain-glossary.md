...
| `G-130` | Storage area              | A mode-less bounded floor region that is immediately usable as one general-storage stock address and may contain optional exact positions or fixtures (`ADR-0018`). | Persisted in `storageZones`; “zone” is retained as a compatibility name.                |
| `G-131` | Storage position          | An optional stable, more precise stock address inside a storage area. Required for rack slots and any area policy that requires exact-child addressing.             | Rejected: forcing one separately configured position for every open Area.               |
| `G-132` | General area stock        | Stock posted to the area's own address, meaning its exact subposition is not recorded. It is separate from, not a rollup of, child-position balances.               | Existing area IDs and QR labels retain this meaning through migration.                  |
| `G-133` | Open-floor position       | An optional named marker or grid cell inside an area with exact X/Y geometry and no arbitrary Z.                                                                    | Examples: `P-12`, `GRID-B4`.                                                            |
| `G-134` | Rack position             | A leaf generated from fixture/rack identity plus bay, level, and slot; its elevation is derived from level.                                                         | Rejected: free-form rack elevation.                                                     |
| `G-135` | Platform position         | An optional exact address on a raised-platform support surface whose elevation is inherited from the platform.                                                      | The platform fixture itself is not automatically a stock address.                       |
| `G-136` | Stock address             | The single location ID recorded by a physical balance and ledger line: either a general Storage Area address or an optional exact child position.                   | Rejected: “Position only” as a universal ledger invariant.                              |
