...
synchronization, and the permission catalogue, policy evaluator, and provisioning
seed. **No warehouse management workflow exists yet**, so there is no feature
function for the enforcement to guard, and a denied _read_ is still not recorded
(`RG-071`). Accordingly, no product outcome claims more than `Partial`.
...
| `SC-O03` | Scan an item; capture quantity, UOM, lot, manufacture/expiry; build a pallet HU           | `convex/handlingUnits/**`, `convex/model/gs1/**`, `src/components/scanner/**` | unit, property, integration, e2e         | [ADR-0005](./adr/0005-warehouse-location-and-stock-identity.md)                                                                | Not implemented |
...
| `SC-B06` | Traceability: `NONE`/`LOT`, serial-ready, HUs | [ADR-0005](./adr/0005-warehouse-location-and-stock-identity.md)                                                         | property, integration tiers   | Not implemented |
