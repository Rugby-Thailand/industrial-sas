...
## Full-flow Phase 1 — shared operator experience

Registered because the [full-flow plan](./plan/full-flow-implementation.md) §8 gives
this phase an exit gate, and a phase with no registered gate is a phase that can be
declared finished in a commit message. `RG-073` is the code half; the operator
evidence in `RG-074` is not obtainable from this repository at all, and `RG-030`
already owns the policy decision behind step-up scope.

| ID       | Gate                                                                                                                                                       | Kind     | Owner            | Evidence required                                                                                                                                                         | Status                                                                                                                                                                                                                                                                                                                                                                          |
| -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `RG-073` | Shared task ownership, partial evidence, quantity capture, device lifecycle, on-device step-up, and offline classification enforced server-side and tested | Code     | Engineering lead | Green unit, integration, isolation, component, and a11y suites over `convex/model/platform/**`, `convex/platform/**`, and the operator surfaces                           | In progress — claim/lease/heartbeat/release/reassignment with retained evidence and a paged activity timeline, maker-checker task exception reporting/resolution, device lifecycle, exact quantity capture, single-use approval, offline classification, and tenant-bound scan evidence are green locally; camera/hardware proof, task attachment viewer, and outbox are absent |
| `RG-074` | A full shared-operator journey completes on target hardware, keyboard-only, in Thai, on a degraded network                                                 | External | Product owner    | Recorded walkthrough per [full-flow plan §17](./plan/full-flow-implementation.md), with scan-to-acknowledgement latency, completion rate, and zero accessibility failures | Not started — no deployment, no identity provider, and no handheld have been exercised; also gated by `RG-002`, `RG-010`, `RG-038`, and `RG-039`                                                                                                                                                                                                                                |
