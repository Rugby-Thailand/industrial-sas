...
interface CustomerOrderLineRow {
  readonly _id: string;
  readonly orgId: TenantOrgId;
  readonly orderedQuantity: number;
}

interface MasterCardRevisionRow {
  readonly _id: string;
  readonly orgId: TenantOrgId;
...
export const createProductionOrder = mutationWithOrg({
  ...
  handler: async (ctx, args) => {
    ...
    const [line, revision] = await Promise.all([
      ctx.tenantDb.get<CustomerOrderLineRow>(
        "customerOrderLines",
        packet.customerOrderLineId,
      ),
      ctx.tenantDb.get<MasterCardRevisionRow>(
        "masterCardRevisions",
        packet.masterCardRevisionId,
      ),
    ]);
    if (line === null || revision === null) {
      return refusal({ code: "REFERENCE_NOT_FOUND", field: "factoryPacketId" });
    }
    ...
    const route = revision.specification.route ?? [];
    if (route.length === 0) {
      return refusal({
        code: "PRECONDITION_FAILED",
        field: "route",
    ...
    let target = line.orderedQuantity * 1_000;
    ...
    const materials = revision.specification.materials ?? [];
